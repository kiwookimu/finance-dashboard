...
상단 신호등은 공개 숫자 데이터가 있는 지표를 점수화합니다. 주가지수의 단기 추세와 1/3/6/12개월 모멘텀, SOX와 메모리 가격을 묶은 반도체 사이클, 공포·탐욕, VIX, VIX와 S&P 500 22거래일 실현변동성 차이로 계산한 변동성 프리미엄, 미국 10년물 금리, 달러/원, WTI, 하이일드 스프레드, NFCI를 반영합니다. 점수가 높고 위험 조건이 과열되지 않으면 녹색불(`신규 매수`), 중립이면 노란불(`홀딩`), 점수가 낮거나 VIX가 30 이상이면 빨간불(`매도`)로 표시합니다. 숫자가 비공개인 DXI와 일부 계약가격은 상태 표시는 하되 점수에는 강하게 반영하지 않습니다.
...
포트폴리오 신호에는 보유 ETF의 28일 상대강도와 1/3/6/12개월 장기 상대강도, 50일/200일 이동평균 위치와 정배열, Naver Finance 기준 최근 5일/20일 외국인·기관 순매수 강도, 하이일드 스프레드와 Chicago Fed NFCI 기반 시장 레짐, 반도체 사이클, 변동성 프리미엄을 함께 반영합니다. 녹색불은 `분할 매수`, 노란불은 `보유`, 빨간불은 `비중 축소`로 표시하며, 점수의 강도는 별도 신뢰도 문구로 표시합니다.
...
세 번째 인자는 기본 선행수익률을 계산할 거래일 수입니다. 현재 백테스트는 5/20/60거래일 선행수익률을 함께 저장하고, 각 신호의 신뢰도도 결과에 포함합니다. 결과는 `screen_results/backtest_signals_*.json`과 `.csv`로 저장됩니다. DDR5 spot, 서버 DRAM 계약가, DXI, 과거 외국인·기관 수급처럼 전체 과거 데이터가 공개 경로로 안정적으로 제공되지 않는 항목은 백테스트에서 제외하거나 가능한 범위만 반영합니다.
